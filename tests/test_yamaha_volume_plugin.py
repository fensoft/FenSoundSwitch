...
class YamahaVolumePluginTests(unittest.TestCase):
    ...
    def test_activation_powers_on_and_selects_input_once(self) -> None:
        fake = FakeSocket([])
        plugin = yamaha.YamahaVolumePlugin()
        plugin._config = yamaha.ReceiverConfig("receiver", power_on=True, startup_input="HDMI1")
        with patch("plugins.yamaha_volume_plugin.socket.create_connection", return_value=fake) as connect:
            plugin.activate_volume_provider()
            plugin.activate_volume_provider()
        connect.assert_called_once()
        self.assertEqual(fake.sent, [b"@MAIN:PWR=On\r\n", b"@MAIN:INP=HDMI1\r\n"])
        self.assertTrue(fake.closed)
