...
MAX_LINE_SIZE = 256
INPUT_OPTIONS = (("Do not change input", ""),) + tuple((value.replace("/", " / ").title(), value) for value in (
    "PHONO", "CD", "TUNER", "DVD", "BD", "TV", "SAT/CBL", "DVR", "GAME", "GAME2", "V.AUX", "DOCK",
    "HDRADIO", "IPOD", "RHAPSODY", "NAPSTER", "PANDORA", "LASTFM", "FLICKR", "FAVORITES", "IRADIO",
    "SERVER", "USB DIRECT", "IPOD DIRECT", "USB", "IPD", "NET", "BT", "MPLAY", "AUX1", "AUX2", "AUX3",
    "AUX4", "AUX5", "AUX6", "AUX7", "QUICK1", "QUICK2", "QUICK3", "QUICK4", "QUICK5", "SMART1",
    "SMART2", "SMART3", "SMART4", "SMART5",
))
...
class DenonMarantzVolumePlugin:
    ...
    def validate_route_output_form(self, host: str, port: str, power_on: object = False, startup_input: object = "") -> dict[str, object]:
        ...
        if config is None:
            raise ValueError("Enter a valid host, TCP port, power option, and Denon/Marantz input.")
    ...
    def get_route_output_ui(self, parameters: Mapping[str, object]) -> dict[str, object]:
        ...
        return plugin_ui_document("Configure Denon/Marantz AVR route", [
            ...
            {"id": "startup_input", "type": "select", "label": "Input on activation", "value": values["startup_input"], "options": [{"label": label, "value": value} for label, value in INPUT_OPTIONS], "description": "The list spans known Denon and Marantz models; the receiver must support the selected source."},
    ...
    def invoke_ui_action(self, action_id: str, values: Mapping[str, object]) -> dict[str, object]:
        if action_id != "save":
            raise ValueError(f"Unknown Denon/Marantz UI action {action_id!r}.")
    ...
    def activate_volume_provider(self) -> None:
        ...
        commands = []
        if config.power_on:
            commands.append(b"PWON\r")
        if config.startup_input:
            commands.append(f"SI{config.startup_input}\r".encode("ascii"))
        with self._lock:
