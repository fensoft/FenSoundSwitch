...
| `plugins/keyboard_input_plugin.py` | Defines two route-scoped passive keys; duplicate combinations are rejected rather than broadcast. |
...
| `core_audio.py` | Minimal ctypes Core Audio adapter for render endpoint discovery and endpoint master volume; it never changes endpoint visibility, names, or defaults. |
