...
7. It independently starts the tray controller and low-level global media-volume hook. Tk publishes immutable menu snapshots to the tray, while tray Refresh/selection/restore/exit actions enqueue Tk callbacks. Volume Down/Up repeat as deltas; Mute dispatches once per physical press only when a ready routed provider advertises confirmed native mute. Their failures remain nonfatal to the other subsystems.
...
Source and installed standalone execution use one interactive process per Windows session, enforced by `SingleInstanceGuard` and the named `Local\fensoundswitch-single-instance` mutex. The Nuitka executable runs directly from its installed directory and has no one-file bootstrap or runtime extraction phase.
...
| `plugin-<id>` | Short-lived daemon per accepted direct trigger | Runs trusted plugin trigger code | Reports status through the host context; never calls Tk directly |
| `plugin-signal-<id>` | Short-lived daemon per accepted action signal | Runs action and interruptible wait slots synchronously in persisted order | Stops on failure or shutdown; repeated runs of the same signal are suppressed |
...
| `bluetooth_audio.py` | Discovers physical paired Windows Bluetooth devices and resolves a configured device's active render endpoint when volume is used. |
...
| `localization.py` | Holds process-local native tray and overlay translations synchronized to the resolved UI language. |
...
| `plugins/denon_marantz_volume_plugin.py` | Bundled Denon/Marantz AVR main-zone provider using bounded TCP power, input, and `MV` commands. |
...
| `plugins/windows_default_device_plugin.py` | Provides four signal-only slots for active Windows playback, voice output, input, and microphone devices; each applies the next device only to its corresponding default roles. |
| `plugins/onkyo_volume_plugin.py` | Bundled main-zone Onkyo/Integra eISCP provider with bounded route-scoped power, input, and volume control. |
| `plugins/pioneer_elite_volume_plugin.py` | Bundled Pioneer/Elite main-zone provider using bounded TCP power, function, and volume control. |
| `plugins/sony_volume_plugin.py` | Bundled Sony network AVR main-zone provider using bounded system, AV-content, and audio JSON-RPC requests. |
| `plugins/yamaha_volume_plugin.py` | Bundled Yamaha main-zone provider using bounded TCP YNCA power, input, and volume commands. |
...
| `plugins/macos_overlay_plugin.py`, `plugins/macos_overlay/assets/*.svg` | Distinct macOS-inspired renderer and plugin-owned packaged route-type/volume/status icon geometry sharing the focus-safe Windows presentation path. |
| `plugin_api.py` | Defines API version 4, `HotkeySpec`, `SlotAction`, optional input, volume-provider, signal-slot, and selectable overlay-renderer capabilities, plus a Tk-safe plugin text-overlay host callback. |
...
| `plugins/mqtt_input_plugin.py` | Persists reusable named broker profiles, creates one bounded client per configured route or automation trigger, routes callbacks through host dispatch boundaries, and publishes retained Home Assistant slider/button discovery. |
| `plugins/windows_application_volume_plugin.py` | Exact-executable Core Audio session volume and confirmed mute on one selected render endpoint. |
| `plugins/http_volume_plugin.py` | Strict bounded HTTP/HTTPS JSON volume provider with redirect rejection and confirmed readback. |
| `plugins/system_automation_plugin.py` | Exact-monitor DDC brightness/contrast, bounded HTTP, and confirmed Windows power-plan signal actions. |
...
| `plugins/windows_bluetooth_volume_plugin.py` | Selects a paired Bluetooth device while connected or disconnected, then resolves and reads/writes only its active endpoint in route workers. |
...
| `plugins/ddc_input_source_plugin.py` | Discovers stable DDC monitor identities and advertised inputs off Tk, then provides a verified monitor-input automation action. |
| `core_audio.py` | Minimal ctypes Core Audio adapter for endpoint/session discovery, endpoint and exact-executable session volume/mute, explicit default-role changes, and silent shared-WASAPI playback; it never changes endpoint visibility or names. |
...
`plugin_manager.discover_plugins()` directly imports API-v4 first-party modules from the bundled `plugins` package in deterministic platform-aware order. It includes overlays; Discord, DDC, system automation, and Windows audio/device actions; AVR and HTTP outputs; and Windows media-key, keyboard, MQTT, Bluetooth, soundcard, microphone, and application-session routes. It then scans sorted `external-plugins\*.py` beside the source/executable, sorted `%APPDATA%\fensoundswitch\plugins\*.py`, and the legacy trusted `%APPDATA%\windows-ddc\plugins\*.py` as a final read-only fallback. The bundled package is never dynamically scanned. Repeated physical external directories are collapsed. A candidate module must expose `PLUGIN_API_VERSION = 4` and `create_plugin()`; malformed objects, later duplicate IDs, import exceptions, and initialization exceptions become visible failure records. The host persists disabled action-plugin IDs in `%APPDATA%\fensoundswitch\plugin-settings\action-plugin-state.json`; a disabled action plugin stays visible but does not initialize or register shortcuts until enabled. There is no sandbox or runtime reload: every discovered external file is deliberately trusted and executes in-process at primary startup.
...
Each receiver route may issue its configured power-on and input-selection commands once from `activate_volume_provider()` before its first availability/read probe. This happens on the serialized worker lane; definition-plugin `initialize()` never performs receiver network I/O. Missing parameters preserve the previous no-startup-action behavior.

Volume outputs may opt into native mute by declaring `supports_native_mute is True` and implementing `toggle_mute() -> bool`. The host requires a real boolean confirmation and runs mute through the same serialized provider-operation lane as volume. Providers without both declarations are never invoked for mute.

The active runtime contract is plugin API v4. Any older v3 wording in historical design descriptions is obsolete.

Schema-v11 action signals compose declared `SlotAction` values with ordered waits and app-start, keyboard, tray, and/or plugin-owned triggers. Slot declarations carry an optional chooser description while remaining source-compatible with two-argument API-v4 declarations. The web editor uses documented modal choosers for triggers and actions, includes Wait in the action chooser, keeps inserted action types fixed, and rejects duplicate trigger types before mapping trigger rows back to the compatible fields. MQTT trigger records reference one reusable profile and carry per-automation Home Assistant names and IDs. The composition root dispatches app-start signals once, after plugin and renderer initialization; duplicate processes never reach this boundary. One `plugin-signal-<id>` worker executes each accepted run synchronously through `run_slot(action_id, parameters)`, suppresses another run of that same signal, stops on the first raised slot failure, and uses the manager shutdown event for interruptible waits. Discord and Windows device switching expose slots only; obsolete direct shortcut records are ignored rather than converted into automations.

The DDC monitor-input plugin exposes one independently configurable `select-input` slot and no Integrations-page form. Opening the action-step editor immediately starts a daemon discovery worker and returns a loading document that the WebView polls until stable monitor/input choices are ready; the editor also owns the explicit refresh command. No discovery work runs on Tk. Windows discovery acquires each physical DDC handle and its `\\.\DISPLAYn` identity inside the same `EnumDisplayMonitors` callback, then constructs the pinned monitorcontrol wrapper around that owned handle. It never correlates independently ordered lists or descriptions. The editor persists only one stable `SavedMonitorSelection` and one advertised numeric input value in each slot's parameters. Dependent select fields keep input choices scoped to that slot's selected monitor. A slot run freshly enumerates monitors, exact-matches the saved identity, rereads capabilities, writes VCP input-source code `0x60` once, and verifies the readback. Missing identities, ambiguous matches, removed inputs, and uncertain readbacks fail without fallback or retry.

The current composition extends that original Routes/Plugins ownership: the manager builds the Routes, Actions, and Appearance pages, while `gui.py` owns Settings, Diagnostics, and the version-backed About page. Routes and Actions use bounded scrollable card lists. The route wizard renders endpoint action metadata, retains replacement documents returned by those actions, and automatically invokes only actions that explicitly declare `auto: true`; Bluetooth discovery uses this opt-in behavior. Routes and Appearance register synchronized views of the one persisted overlay renderer, and every visible Start with Windows control is refreshed from the same GUI-owned state after a mutation.

Host-published `VolumeStatus` snapshots carry the configured route type alongside the route name and confirmed volume. The macOS renderer maps every supported type (`voice`, `headset`, `headphones`, `earbuds`, `speakers`, `soundbar`, `tv`, `avr`, `amplifier`, `microphone`, `line-in`, `line-out`, `mixer`, `monitor`, and `other`) to a plugin-owned SVG instead of inferring device semantics from user-defined names.
...
- `enumerate_monitor_inputs()` reads and normalizes the input values advertised for VCP code `0x60`.
- `set_monitor_input()` rechecks the advertised values, writes one selected input, and requires a matching readback without retrying an uncertain write. `set_monitor_input_if_needed()` exposes whether a write occurred so an already-active automation step remains silent.

A process-wide DDC operation lock prevents capability, input, and volume operations from entering monitorcontrol concurrently across route and automation workers.
...
Routed inputs use the same single operation lane with separate Tk-owned presentation state. Each accepted Windows-volume delta, keyboard-route repeat, or absolute route target updates `_route_presentation_targets` and refreshes an ephemeral overlay snapshot on the next 50 ms queue poll. These requested values never enter `_volume_statuses`, the tray snapshot, or plugin status callbacks, which remain authoritative read/readback state. While a provider is busy, deltas accumulate per route and absolute inputs collapse to the latest target; completion clears only the presentation target it fulfilled, so an older readback cannot visually overwrite newer queued intent. Fast receiver writes retain their optimistic baseline until key-release reconciliation. Topology invalidation, route cancellation, failure without newer intent, and authoritative reconciliation discard transient presentation state.
...
When `MonitorVolumeApp._should_consume_volume_keys()` is false, Volume Down/Up pass onward. Mute uses a separate readiness check and passes onward unless at least one ready Windows-media-key route has a provider with `supports_native_mute is True` and callable `toggle_mute()`. If a previously configured target is unavailable, the first volume key-down in that invalid period also queues an error overlay without consuming the event. The notice is latched until readiness is restored or a new invalidation begins. On a consumed press, matching key-up retains the initial consume decision; readiness loss does not split a press between the app and Windows. Volume Down/Up repeat as deltas, while Mute toggles only on the first key-down. The hook's numeric step is updated through a lock-protected setter when Tk handles the persistent Slow (`+1`), Medium (`+2`), or Fast (`+3`) Change speed event, so the native thread never reads Tk state.
...
The public Core Audio endpoint-property contract supports reading but makes the non-administrator property store read-only; that is why the normal process never attempts the rename directly. The Windows application-volume provider changes only an exact executable's active session volume/mute on its configured endpoint and never changes endpoint visibility, names, or defaults. Relevant Microsoft contracts are [audio endpoint properties](https://learn.microsoft.com/en-us/windows/win32/coreaudio/device-properties), [`IMMDevice::OpenPropertyStore`](https://learn.microsoft.com/en-us/windows/win32/api/mmdeviceapi/nf-mmdeviceapi-immdevice-openpropertystore), and [audio endpoint container IDs](https://learn.microsoft.com/en-us/windows-hardware/drivers/audio/audio-endpoint-container-id).
...
- Context menu status: routing enabled/disabled
- Context menu actions: Restore (`1001`), Exit (`1002`), Refresh (`1003`), and up to 100 configured automation commands starting at `3000`

Tk builds `TrayMenuState` from `_hotkeys_enabled` and configured tray-triggered automations, then replaces the controller's snapshot under a lock whenever those values change. The tray thread copies the snapshot when the popup opens, so a returned automation command maps to the snapshot that created the menu. Labels normalize whitespace, escape Win32 mnemonic ampersands, and are bounded to 96 characters.

Refresh and automation callbacks never invoke Tk directly. The controller calls closures that enqueue `refresh_configured_routes()` or automation dispatch through `_post_to_ui()`. Routing reflects the same `_hotkeys_enabled` state used by the native hook.
...
Minimizing a visible Tk window schedules an idle check and withdraws it only if the state is `iconic`. Restoring keeps the tray icon available while normalizing/lifting/focusing the Tk window and reapplying dark title-bar chrome. Closing the visible window follows the full shutdown path rather than minimizing.

## Frontend and overlay

The control window is vertically resizable with a DPI-scaled 900-logical-pixel minimum width. Its persistent sidebar switches among Routes, Automations, Integrations, Appearance, Settings, Diagnostics, and About. Routes presents live route cards and the six-step route wizard. Automations owns trigger and ordered-step composition. Integrations owns reusable MQTT profiles and plugin-wide setup. Appearance owns overlay preview, renderer selection, and renderer settings; its Test action sends two synthetic `VolumeStatus` rows at 30% and 70% through the same selection and `show_statuses` path as live routed updates. Settings owns startup, language, and configuration archives. The packaged `web/i18n.js` catalog provides English, German, Spanish, French, and Italian command-center text with English fallback; hardware names, user text, raw diagnostics, and unknown external-plugin strings remain opaque. Unmapped pages do not participate in keyboard traversal. Tray Refresh remains the explicit status rediscovery action; the main window has no Refresh button.
...
The manually maintained real-device captures are tracked at `docs/app.png` (452×203) and `docs/overlay.png` (210×122). They predate the route-page navigation, Fluent visual system, live theme/scaling behavior, and current error presentation. The user manual also contains scrubbed reference images rendered from the current packaged front end with fictional snapshot data. There is no automated screenshot workflow; replacing the real-device captures still requires real scrubbed application captures.
...
The monitor-selection loader accepts the old unversioned description/ordinal shape for safe one-time promotion, rejects boolean ordinals, and treats missing files, I/O failures, invalid JSON, non-object roots, unknown versions, and invalid nested data as no selection. Change speed is an independent top-level preference: missing or invalid values default to `slow`, and valid values are `slow`, `medium`, and `fast`.

Route, signal, and language persistence uses schema version 12. `ui_language` stores `auto`, `en`, `de`, `es`, `fr`, or `it`; automatic mode resolves the Windows user locale's primary language and falls back to English. Each ordered `volume_routes` record has a stable `route_id`, a validated user-defined `name`, a presentation-only type enum, and JSON input/output endpoints. Older records default safely to `other`. Each ordered `action_signals` record has a stable ID, validated name, app-start/keyboard/tray/plugin triggers, and one to 32 ordered plugin-action or bounded wait slots. MQTT profiles are plugin-owned JSON and are referenced by stable IDs from routes and triggers. Schema-v6 route records are read safely with a deterministic `<input ID> to <output ID>` name and are upgraded only when settings are next saved, preserving unrelated settings. Route names identify independent output instances in the Routes table, status messages, and all overlay rows.
...
Plugin-owned settings live under `%APPDATA%\fensoundswitch\plugin-settings`; MQTT profiles may contain broker credentials, while Discord OAuth client configuration and tokens remain Credential Manager-only. First-party executable code lives in the bundled `plugins` package and is imported directly. External executable code is discovered separately under `external-plugins` beside the source/executable and `%APPDATA%\windows-ddc\plugins`.
...
```text
app, app_version, audio_outputs, autostart, core_audio, diagnostics, gui, localization, ddc, settings, theme, plugin_api, plugin_hotkeys, plugin_manager, native_platform, macos_platform, macos_plugin_hotkeys, windows_platform, web_presentation, web_ui_host
```

Direct runtime dependencies pin `Pillow==10.1.0`, `monitorcontrol==4.2.0`, `paho-mqtt==2.1.0`, `pywebview==6.1`, and `pythonnet==3.0.5`. Pillow supersamples the strict packaged `plugins/macos_overlay/assets/*.svg` icon geometry and overlay text before Lanczos downsampling; pywebview and pythonnet host the local Windows WebView2 presentation child. Setuptools package data and the Nuitka build both include that plugin-owned asset directory. The `build` extra pins `Nuitka==4.2`. There is no lockfile; build-system requirements and transitive build dependencies are not fully pinned.
...
`build_exe.ps1` resolves a 64-bit `python`, changes to its own repository directory, checks `app.py` and `FenSoundSwitch.ico`, then creates a temporary ignored `fensoundswitch-version.txt` build resource and invokes Nuitka. The default build version is `dev`. Tag publication accepts `major.minor` or `major.minor.patch` with an optional leading `v` and passes the exact `github.ref_name`; `app_version.py` reads that bundled resource for the About page, while source execution falls back to `dev`. Numeric tag components are padded for Windows file/product metadata and normalized to three fields for MSI upgrades. The temporary resource is removed in `finally`. The MSI uses the same icon for Add/Remove Programs and explicitly assigns it to the advertised Start Menu shortcut.

- `--standalone` so the installed executable has no self-extracting bootstrap
...
Nuitka compiler intermediates are isolated in a unique system temporary directory to avoid synchronized-folder and antivirus file locks. Only after compilation succeeds does Windows `robocopy /MIR` synchronize the completed tree into ignored `dist\FenSoundSwitch\`, avoiding delete failures from synchronized-folder directory handles. `build_installer.ps1` uses pinned WiX 6.0.2 and `installer\FenSoundSwitch.wxs` to harvest that standalone tree into `dist\FenSoundSwitch.msi`, then runs Windows Installer ICE validation. The native per-machine MSI installs under Program Files, creates an all-users Start Menu shortcut, supports major and same-version replacement through a stable UpgradeCode, and removes application files without deleting per-user settings or credentials. It cannot safely remove each profile's optional HKCU autostart value, so users should disable Start with Windows before uninstalling. It has no executable installer bootstrap. The Release workflow verifies a `dev` MSI for `master` pushes without uploading it; a pushed version tag embeds that tag and publishes the MSI to the matching GitHub release. Releases remain unsigned.
...
`.github/workflows/ci.yml` runs on `windows-latest` for pushes, pull requests, and manual dispatches with Python 3.10. It installs the runtime project, runs the unit suite, compiles runtime modules, checks installed dependencies, parses both build scripts and the WiX source without executing them, checks tracked/staged whitespace, and requires a clean repository state. It neither starts `app.py` nor invokes hardware tools, live audio enumeration/mutation, monitor enumeration, installer builds, publishing, or deployment. `.github/workflows/release.yml` builds and verifies the standalone application and MSI on `master`, publishing the MSI only for pushed tags. `tests/test_ci_workflow.py` locks down both workflow contracts.
...
- Keep tray state immutable and lock-protected, and keep the menu-open snapshot paired with its automation command IDs. Tray actions must enter Tk through `_post_to_ui()`.
...
- Never touch the live Run value in automated work. Preserve current-user-only opt-in writes, Windows quoting, the command-length check, source `pythonw.exe` preference, and installed standalone executable path.
