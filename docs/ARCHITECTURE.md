...
Schema-v10 action signals compose declared `SlotAction` values with ordered waits and app-start, keyboard, tray, and/or plugin-owned triggers. Slot declarations carry an optional chooser description while remaining source-compatible with two-argument API-v4 declarations. The web editor uses documented modal choosers for triggers and actions, includes Wait in the action chooser, keeps inserted action types fixed, and rejects duplicate trigger types before mapping trigger rows back to the compatible schema-v10 fields. MQTT trigger records reference one reusable profile and carry per-automation Home Assistant names and IDs. The composition root dispatches app-start signals once, after plugin and renderer initialization; duplicate processes never reach this boundary. One `plugin-signal-<id>` worker executes each accepted run synchronously through `run_slot(action_id, parameters)`, suppresses another run of that same signal, stops on the first raised slot failure, and uses the manager shutdown event for interruptible waits. Discord and Windows device switching expose slots only; obsolete direct shortcut records are ignored rather than converted into automations.
