...
Route and signal persistence uses schema version 9. Each ordered `volume_routes` record has a stable `route_id`, a validated user-defined `name`, and JSON input/output endpoints. Each ordered `action_signals` record has a stable ID, validated name, keyboard and/or tray trigger, and one to 32 ordered plugin-action or bounded wait slots. Schema-v6 route records are read safely with a deterministic `<input ID> to <output ID>` name and are upgraded only when settings are next saved, preserving unrelated settings. Route names identify independent output instances in the Routes table, status messages, and all overlay rows.
