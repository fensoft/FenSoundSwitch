...
Schema-v10 action signals compose declared `SlotAction` values with ordered waits and app-start, keyboard, tray, and/or plugin-owned triggers. MQTT trigger records reference one reusable profile and carry per-automation Home Assistant names and IDs. The composition root dispatches app-start signals once, after plugin and renderer initialization; duplicate processes never reach this boundary. One `plugin-signal-<id>` worker executes each accepted run synchronously through `run_slot(action_id, parameters)`, suppresses another run of that same signal, stops on the first raised slot failure, and uses the manager shutdown event for interruptible waits. Discord and Windows device switching expose slots only; saved direct bindings for those bundled plugins migrate to equivalent one-slot signals.
...
- `set_monitor_input()` rechecks the advertised values, writes one selected input, and requires a matching readback without retrying an uncertain write.
