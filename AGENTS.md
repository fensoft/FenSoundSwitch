...
- Runtime dependencies: `Pillow==10.1.0`, `monitorcontrol==4.2.0`, `paho-mqtt==2.1.0`, `pywebview==6.1`, and `pythonnet==3.0.5`. Optional executable builder: `Nuitka==4.2`.
...
- The global Volume Down/Up hook, passive plugin shortcut observer, Discord voice-output changes, physical DDC volume/input/brightness/contrast writes, application-session and endpoint changes, outbound HTTP actions, power-plan changes, and elevated FenSound rename are safety-sensitive. Do not launch the app or call live monitor/audio/network/Discord/power operations as routine automated validation.
...
4. `display-change-listener`, `tray-icon`, and `volume-key-hook` are long-lived daemon threads with native Win32 message loops. The display listener also relays live theme/system-color broadcasts into Tk. The native hook handles Volume Down/Up and one-shot Mute only for ready capable routes. The plugin-hotkey observer is independent: action shortcuts and keyboard routes may explicitly consume only their configured held key pairs.
...
9. Confirmed tray-icon addition makes startup tray-first unless `--foreground` immediately invokes the existing restore path. Tk publishes immutable routing and action-signal snapshots; tray actions queue back into Tk. Addition or recovery failure keeps/restores the main window; Explorer recreation re-adds a previously visible icon. Closing the restored window exits, while minimizing returns it to the tray after another confirmed add.
...
| `core_audio.py` | Focused Core Audio endpoint/session enumeration and volume adapter for soundcard, capture-gain, and exact-application route workers, explicit default-role changes, and silent WASAPI keep-alive streams. |
...
| `localization.py` | Process-local native tray and overlay translations synchronized to the resolved UI language. |
...
| `plugins/windows_application_volume_plugin.py` | Bundled exact-executable Core Audio session volume and confirmed mute provider. |
| `plugins/http_volume_plugin.py` | Bundled bounded HTTP/HTTPS JSON volume provider with redirect rejection and confirmed readback. |
| `plugins/system_automation_plugin.py` | Bundled exact-monitor DDC brightness/contrast, bounded HTTP request, and confirmed Windows power-plan automation slots. |
...
| `plugins/macos_overlay/assets/*.svg` | Plugin-owned packaged source geometry for all route-type, volume, success, and error icons used by the macOS overlay. |
...
- New settings use schema version 12 and persist the UI language preference (`auto`, `en`, `de`, `es`, `fr`, or `it`), ordered independent route records (`route_id`, user-defined `name`, informational `route_type`, input endpoint, output endpoint), and ordered action signals. Automatic language follows the supported primary subtag of the Windows user locale and falls back to English. Each signal has app-start, keyboard, tray, and/or plugin trigger records and one to 32 synchronous plugin-action or bounded wait slots. MQTT trigger records reference a plugin-owned named profile and keep their own Home Assistant name/ID. Older routes default to type `other`; schema-v6 routes also receive deterministic endpoint-ID names when next saved. Overlay selection and renderer settings live under `plugin-settings`; legacy global `overlay_mode` migrates to the Windows 11 renderer. Legacy Change speed/active-provider fields remain where present, along with description, a Windows device path, and optional EDID manufacturer/product/serial identity. Never persist the transient display index.
...
- Optional routed output mute capability: `supports_native_mute is True` and `toggle_mute() -> bool`
...
Keep each per-monitor DDC get/set sequence inside `with monitor_ref.monitor:`. On Windows, enumeration must acquire each physical DDC handle and its display identity in the same `EnumDisplayMonitors` callback, then construct the pinned monitorcontrol wrapper around that owned handle; never correlate independent inventories by list order or description. Description lookup remains on the wrapper. Preserve `0`–`100` application clamping and treat the post-write readback as authoritative when it succeeds.
...
- tray routing state, Refresh, automation actions, Restore, and Exit;
...
- Keep plugin shortcut observation separate from `GlobalVolumeKeyListener`: action shortcuts, action signals, and keyboard routes may consume only their configured non-modifier key down/repeat and matching held key up when their persisted `forward_keys` setting is false; modifier and unrelated keys always forward. Existing action shortcuts migrate to forwarding. Queue dispatch away from the native thread, execute signal slots synchronously only on their manager worker, make waits shutdown-interruptible, suppress held-key repeats and overlapping action triggers, pass native repeated key-down events through for keyboard-route decrease/increase, and keep route mute one-shot.
...
- Keep native mute on the same serialized provider-operation slot as volume. Consume Mute only for ready explicitly capable routes, toggle once per physical press, preserve the consume decision through key-up, and leave DDC or other unsupported outputs unchanged.
...
- Preserve immutable lock-protected tray snapshots and dispatch automation command IDs against the snapshot that built the open menu. Never call Tk from the tray thread; tray actions must enqueue through `_post_to_ui()`.
